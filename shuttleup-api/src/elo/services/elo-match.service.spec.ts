...
const makeMockTx = () => ({
  eloMatch: { create: jest.fn().mockResolvedValue({ id: 'match-1', sessionId: 'session-1', gameType: 'singles', score: '2-0', status: 'CONFIRMED', playedAt: new Date(), createdAt: new Date() }) },
  eloMatchParticipant: { createMany: jest.fn().mockResolvedValue({ count: 2 }) },
...
const makePrismaMock = () => {
  ...
  return {
    ...
    $transaction: jest.fn().mockImplementation((fn: (tx: typeof tx) => Promise<unknown>) => fn(tx)),
  };
};
...
describe('EloMatchService', () => {
  ...
  describe('submitMatch — singles', () => {
    beforeEach(() => {
      prisma.courtSession.findUnique.mockResolvedValue(mockSession(HOST_ID, SESSION_ID));
  ...
  describe('submitMatch — authorization', () => {
    it('throws ForbiddenException if not host', async () => {
      prisma.courtSession.findUnique.mockResolvedValue(mockSession('other-host', SESSION_ID));
      await expect(service.submitMatch(SESSION_ID, singlesDto, HOST_ID)).rejects.toThrow(
        ForbiddenException,
      );
    });
  ...
  describe('submitMatch — validation', () => {
    ...
    it('throws BadRequestException for singles with 2 players per team', async () => {
      ...
      await expect(service.submitMatch(SESSION_ID, badDto, HOST_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('findMatch', () => {
    it('returns match with participants', async () => {
      ...
      prisma.$transaction = jest.fn(); // reset — not needed here
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (prisma as any).eloMatch.findUnique = jest.fn().mockResolvedValue(matchWithParticipants);
    ...
    it('throws NotFoundException for unknown match id', async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (prisma as any).eloMatch.findUnique = jest.fn().mockResolvedValue(null);
      await expect(service.findMatch('non-existent')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getUserMatchHistory', () => {
    it('returns paginated match history', async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (prisma as any).eloMatchParticipant = {
        count: jest.fn().mockResolvedValue(5),
        findMany: jest.fn().mockResolvedValue([{ id: 'p1', match: {} }]),
      };
